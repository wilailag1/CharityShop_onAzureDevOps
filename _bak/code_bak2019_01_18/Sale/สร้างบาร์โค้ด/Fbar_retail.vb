Public Class Fbar_retail
    Dim number As Integer

    Private Sub Button2_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button2.Click
        Randomize()
        number = Int(Rnd() * 999999999) + 1
        txtpid.Text = "*b-" & number.ToString("000000000") & "-c*"
        txtbarcode.Text = txtpid.Text
        txtpname.Focus()
    End Sub

    Private Sub Fbar_retail_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Dim conn As New OleDb.OleDbConnection("Provider=Microsoft.ACE.OLEDB.12.0;Data Source=" & Application.StartupPath & "\dbsub.accdb")
        conn.Open()

        Dim strshow As String = "select ID_pro,p_name,p_type,p_price_cost,p_price_sale,p_unit from T_bar_retail where b_date = '" & DateTimePicker1.Text & "'"
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        Else
            ...
            Dim strinsert As String = "insert into T_pro_retail(ID_pro,p_name,p_type,p_price_cost,p_price_sale,p_num,p_unit,p_bal) values('" & txtpid.Text & "','" & txtpname.Text & "','" & txtptype.Text & "','" & txtpprice_cost.Text & "','" & txtpprice_sale.Text & "','0','" & txtpunit.Text & "','0')"
            Dim cmd As New OleDb.OleDbCommand(strinsert, conn)
            cmd.ExecuteNonQuery()
            Dim strinsert1 As String = "insert into T_bar_retail(ID_pro,p_name,p_type,p_price_cost,p_price_sale,p_unit,b_date) values('" & txtpid.Text & "','" & txtpname.Text & "','" & txtptype.Text & "','" & txtpprice_cost.Text & "','" & txtpprice_sale.Text & "','" & txtpunit.Text & "','" & DateTimePicker1.Text & "')"
